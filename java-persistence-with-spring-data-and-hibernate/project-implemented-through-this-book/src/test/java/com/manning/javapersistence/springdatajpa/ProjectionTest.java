...
public class ProjectionTest extends SpringDataJpaApplicationTests {
  ...
  void testUserSummaryProjection() {
    List<UserProjection.UserSummary> userSummaries = userRepository
        .findByRegistrationDateAfter(LocalDate.of(2021, Month.JANUARY, 1));
    assertAll(
  ...
  void testDynamicProjection() {
    List<UserProjection.UsernameOnly> usernameOnlies = userRepository.findByLevel(1, UserProjection.UsernameOnly.class);
    List<UserProjection.EmailOnly> emailOnlies = userRepository.findByLevel(4, UserProjection.EmailOnly.class);
    assertAll(
