...
class AddressServiceImplTest {
  ...
  private static final String nonExistId = "a1b9b31d-e73c-4112-af7c-b68530f38220";
  ...
  @Test
  @DisplayName(
      "returns an AddressEntity when private method toEntity() is called with AddAddressReq model")
  public void convertModelToEntity() {
    ...
    // then
    then(e).as("Check address entity is returned & not null").isNotNull();
  ...
  public void deleteAddressesByNonExistId() {
    ...
    try {
      ...
      assertThat(e.getMessage())
          .isEqualTo(String.format("No Address found with id %s", nonExistId));
    }
  ...
  @Test
  @DisplayName("return address by given exist address id")
  public void getAddressByExistId() {
    // given
    given(repository.findById(UUID.fromString(id))).willReturn(Optional.of(entity));
  ...
  @Test
  @DisplayName("return empty address by given non-existing id")
  public void getAddressByNonExistId() {
    // given
    given(repository.findById(UUID.fromString(nonExistId))).willReturn(Optional.empty());

    // when
    Optional<AddressEntity> result = service.getAddressById(nonExistId);
